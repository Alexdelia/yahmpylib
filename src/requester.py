...
def requester(
    ...
    """
    make request for a given endpoint and check the right status code has been thrown
    ...
    print(
        ...
        f"\t\t\033[33;1mcode\033[0m:  \t\033[{['31', '32'][q.status_code == status_code]};1m{q.status_code}\033[0m"
        + f"\t\033[3m(expected \033[1;3m{status_code}\033[0m\033[3m)\033[0m",
        f"\t\t\033[33;1mcontent\033[0m:  \t{_format_flat_json(q.content, q)}",
        f"\t\t\033[33;1mtime\033[0m:\t\033[2;3m{q.elapsed.microseconds / 1e+6}\033[0m\033[1;3ms\033[0m",
        # f"\t\t\033[33;1mheaders\033[0m:\t{_format_flat_json(dict(q.headers))}",
        file=stderr,
        sep="\n"
    )

    assert q.status_code == status_code, f"Wrong status code, expected {status_code}, got {q.status_code}\n{q.text}"
